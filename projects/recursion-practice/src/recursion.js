...
// nthFibo(7); // 13
// nthFibo(3); // 2
var nthFibo = function(n, fibArray = []) {
  // base if n is zero return last item from fibArray
  if(n === 0){
  ...
  if(n < 0){
    return null;
    // if fibArray length is zero
  } 

  if(fibArray.length === 0){
    // push 0 to array
    fibArray.push(0);
  // else if fibArray length is 1
  }

  if(fibArray.length === 1){
    // push 1 to array
    fibArray.push(1);
    // else add the last to elements of fibArray and push to fibArray
  } else {
    let newFib = fibArray[fibArray.length - 2] + fibArray[fibArray.length - 1];
    fibArray.push(newFib);
  }

  // return nthFibo decrementing n by 1
  return nthFibo(n - 1, fibArray);
};
...
var capitalizeWords = function(input, output = []) {
  // base - if input's length is zero return output
  if(input.length === 0){
    return output;
  }
  // recursion
  // capitalize zero index of input and push to output
  output.push(input[0].toUpperCase());
  // call capitalizeWords with zero index of index sliced off
...
var capitalizeFirst = function(array, output = []) {
  ...
  // recursion
  let outputStr = array[0][0].toUpperCase() + array[0].slice(1);
...
var numToText = function(str, output = '') {
  ...
  if(output.length === 0){
    ...
    output += 'one';
    // if value of str at 0 index is '3'
  } else if(str[0] == 3){
    // push 'three' to output
    output += 'three';
  // else if value of str at 0 index is '5'
  } else if (str[0] == 5){
    // push 'five' to output
    output += 'five';
  // else if value of str at 0 index is '6'
  } else if (str[0] == 6){
    // push 'six' to output
    output += 'six';
  // else push value of str at 0 to output
  } else {
    output += str[0];
  }
  // return call numToTest with string sliced from 1 index
